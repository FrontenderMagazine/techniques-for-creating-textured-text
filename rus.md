...
значением `border-box` и может принимать другие значения, такие как `padding-
box` и `content-box`. 
...
[Для Webkit][4] для в свойство `background-clip` добавлено четвёртое значение, 
...
Иначе говоря, что мы определяем (`defs`) SVG-маску (`mask`), которая является 
